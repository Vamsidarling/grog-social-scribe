 ...
 * 
 * Display card for each social media platform with platform-specific styling,
 * content display area, and action buttons.
 * 
 * Features:
 * - Platform-specific colors and styling
 * - Content display area with scrolling
 * - Action buttons for generation, copying and sharing
...
const PlatformCard: React.FC<PlatformCardProps> = ({ platform, content, isGenerating, onClick, isAuthenticated }) => {
  ...
  const platformConfigs = {
    twitter: {
      ...
      hoverColor: 'hover:border-social-twitter',
      bgColor: 'bg-social-twitter/10',
    ...
    instagram: {
      ...
      hoverColor: 'hover:border-social-instagram',
      bgColor: 'bg-social-instagram/10',
    ...
    facebook: {
      ...
      hoverColor: 'hover:border-social-facebook',
      bgColor: 'bg-social-facebook/10',
      iconBg: 'bg-social-facebook',
    ...
    linkedin: {
      ...
      hoverColor: 'hover:border-social-linkedin',
      bgColor: 'bg-social-linkedin/10',
      iconBg: 'bg-social-linkedin',
  ...
  return (
    <div className={cn(
      "platform-card animate-fade-in scale-in p-4", 
      config.color, 
      config.hoverColor,
      config.bgColor
    )}>
      <div className="flex items-center mb-3 gap-2">
        <div className={`p-1.5 rounded-full ${config.iconBg} text-white shadow-md`}>
          {config.icon}
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button 
          ...
          className="flex-1 text-xs bg-white/50 backdrop-blur-sm dark:bg-gray-800/50 hover:bg-white dark:hover:bg-gray-700"
        >
          Generate
        </Button>
        ...
        {content && (
          <>
            <Button 
              ...
              disabled={isGenerating}
              className="text-xs bg-white/50 backdrop-blur-sm dark:bg-gray-800/50 hover:bg-white dark:hover:bg-gray-700"
            >
              {copied ? (
            ...
            <Button
              ...
              disabled={isGenerating}
              className={`text-xs text-white ${config.iconBg} hover:opacity-90`}
            >
